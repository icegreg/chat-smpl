...
(function($) {
    ...
    function getUserMedia(options) {
        ...
        var errorHandler = options.onerror || function(e) {
        ...
        console.log('[getUserMedia] Secure context:', isSecureContext);
        console.log('[getUserMedia] Protocol:', window.location.protocol);
        console.log('[getUserMedia] Hostname:', window.location.hostname);
        console.log('[getUserMedia] navigator.mediaDevices available:', !!navigator.mediaDevices);
        console.log('[getUserMedia] navigator.mediaDevices.getUserMedia available:', !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia));

        // Use modern mediaDevices API (navigator.getUserMedia is deprecated)
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            console.log('[getUserMedia] Using modern mediaDevices API');
            navigator.mediaDevices.getUserMedia(constraints)
                .then(streaming)
                .catch(handleError);
        } else {
            // Fallback for older browsers or insecure context
            console.log('[getUserMedia] Using legacy API fallback');
            ...
            if (getUserMediaLegacy) {
                getUserMediaLegacy.call(navigator, constraints, streaming, handleError);
            } else {
                // No API available - likely insecure context issue
                var errorMsg = 'getUserMedia is not supported';
                if (!isSecureContext) {
                    errorMsg = 'getUserMedia requires HTTPS or localhost. Current origin (' +
                        window.location.origin + ') is not a secure context. ' +
                        'Please access via HTTPS or localhost.';
                }
                console.error('[getUserMedia] ' + errorMsg);
